...
def load_librarything(args, **kwargs):
...
def load_epinions(args, **kwargs):

    unique_sid = list()
    with open(os.path.join("datasets", "epinions", 'unique_sid.txt'), 'r') as f:
        for line in f:
            unique_sid.append(line.strip())
    ...
    # train, validation and test data
    x_train = load_train_data(os.path.join(
        "datasets", "epinions", 'train.csv'))
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(os.path.join("datasets", "epinions", 'validation_tr.csv'),
                                         os.path.join("datasets", "epinions", 'validation_te.csv'))

    x_test_tr, x_test_te = load_tr_te_data(os.path.join("datasets", "epinions", 'test_tr.csv'),
                                           os.path.join("datasets", "epinions", 'test_te.csv'))
...
def load_dataset(args, **kwargs):
    if args.dataset_name == 'yelp':
        train_loader, val_loader, test_loader, args = load_yelp(args, **kwargs)
    elif args.dataset_name == 'librarything':
        train_loader, val_loader, test_loader, args = load_librarything(
            args, **kwargs)
    elif args.dataset_name == 'epinions':
        train_loader, val_loader, test_loader, args = load_epinions(
            args, **kwargs)
    else:
